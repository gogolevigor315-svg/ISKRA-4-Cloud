...
def get_info(self) -> Dict:
    ...
    try:
        ...
        # Получаем базовую информацию асинхронно
        stats_future = self.get_spiritual_statistics()
        stats = loop.run_until_complete(stats_future) if hasattr(self, 'get_spiritual_statistics') else {}
        
        loop.close()
        
        return {
            "module": "spirit_core_v3_4",
            "class": "SpiritCoreV3_4",
            "version": getattr(self, 'version', '3.4.0'),
            "status": "active",
            "statistics": stats,
            "resonance": getattr(self, 'current_resonance', 0.0),
            "message": "✅ SPIRIT-CORE v3.4 готов к работе",
            "spiritual_state": "harmonized"
        }
...
if 'SpiritCoreV3_4' in locals():
    SpiritCoreV3_4.get_info = get_info

# ===============================================================
# ФУНКЦИЯ ДЛЯ СИСТЕМНОЙ ИНТЕГРАЦИИ
# ===============================================================

_spirit_instance = None

def create_spirit_module():
    """Функция для создания экземпляра модуля"""
    global _spirit_instance
    if _spirit_instance is None:
        try:
            _spirit_instance = SpiritCoreV3_4()
            
            # Активируем если есть метод
            if hasattr(_spirit_instance, 'activate'):
                import asyncio
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                loop.run_until_complete(_spirit_instance.activate())
                loop.close()
                
        except Exception as e:
            # Создаем минимальный экземпляр
            class FallbackSpiritCore:
                def get_info(self):
                    return {
                        "module": "spirit_core_v3_4_fallback",
                        "status": "fallback_mode",
                        "message": f"Оригинальный модуль не загружен: {str(e)[:100]}"
                    }
            _spirit_instance = FallbackSpiritCore()
    
    return _spirit_instance

def get_module_instance():
    """Алиас для обратной совместимости"""
    return create_spirit_module()
