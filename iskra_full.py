...
# ============================================================================
_tree_activated = False # Флаг активации сефиротического дерева
_sephirot_bus = None # Экземпляр шины
_sephirotic_engine = None # Экземпляр движка
activated_nodes = 0 # Количество активированных узлов
current_resonance = 0.0 # Текущий резонанс системы
# ============================================================================
...
# ============================================================================
# ХЕЛПЕРЫ ДЛЯ ЛЕНИВОГО ИМПОРТА (ИЗБЕГАЕМ ЦИКЛИЧЕСКИХ ЗАВИСИМОСТЕЙ)
# ============================================================================
...
async def activate_sephirotic_tree():
    ...
    try:
        ...
        if 'DAAT' not in existing_nodes:
            ...
            # В адаптере все узлы считаются активными
...
print("🔥"*50 + "\n")
# ============================================================================
# СОХРАНЕНИЕ ДЕРЕВА ДЛЯ API
# ============================================================================
# Сохраняем созданное дерево в глобальную переменную, которую видит API
if _tree_activated and _sephirotic_engine and hasattr(_sephirotic_engine, 'tree'):
    # Если есть движок с деревом
    sephirot_tree_for_api = _sephirotic_engine.tree
    print(f"🌳 Дерево сохранено для API из движка (активировано: {_tree_activated})")
elif _tree_activated and 'tree' in locals() and tree:
    # Если есть локальная переменная tree
    sephirot_tree_for_api = tree
    print(f"🌳 Дерево сохранено для API из локальной переменной")
else:
    sephirot_tree_for_api = None
    print(f"⚠️ Дерево не сохранено для API")
# Делаем дерево доступным для эндпоинтов
if sephirot_tree_for_api:
    # Добавляем в глобальную область, где его увидит API
    import sys
    this_module = sys.modules[__name__]
    this_module.sephirot_tree_api = sephirot_tree_for_api
    print(f"✅ Дерево экспортировано в модуль для API")
...
async def initialize_system():
    ...
    if result["status"] == "completed":
        ...
        activated = result.get("sephirot_activated", False)
